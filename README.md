...
Spring Boot로부터 정상 Pod 데이터를 수신하여 ML 모델 학습 데이터로 축적합니다.
데이터가 충분히 쌓이면 백그라운드에서 자동으로 Isolation Forest 모델을 학습합니다.
...
- `sample_count`: 현재까지 누적된 정상 데이터 개수
- `is_trained`: ML 모델 학습 완료 여부
- Spring Boot가 1분마다 호출 권장
...
> 첫 학습 전까지 `/analyze` 호출 시 ML 탐지는 스킵되고 z-score · RAG · LLM은 정상 동작합니다.
...
> ML 모델(`models/isolation_forest.pkl`)은 PVC에 저장되므로
> 컨테이너가 재시작돼도 재학습 없이 기존 모델을 그대로 사용합니다.
